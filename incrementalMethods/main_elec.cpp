...
void mainElec(bool rf, double ir, bool bagging, long nTree){
	long memSize = 1500;
	...
	if(rf){
		RandomForest* test;
		test = new RandomForest(nTree, feature, (int*)isSparse, ir, classes, Evaluation::entropy, bagging);
		for(i = 1; i<944; i++){
		...
		DecisionTree* test;
		test = new DecisionTree(feature, isSparse, ir, feature, classes, Evaluation::entropy);
		long maxSize = 0;
		for(i = 1; i<944; i++){
			//printf("%ld\n", i);
				data = (double**)malloc(no*sizeof(double*));
